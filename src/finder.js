...
let ops = {
  $range() {
    this.op = '$range';
    this.dump = function () {
      return this._args[0].dump() + ' in range (' + [
        this._args[1].dump(),
        this._args[2].dump(),
        this._args[3].dump(),
        this._args[4].dump()
    ...
    this._index = function (index) {
      ...
      return index.range(this._args[2]._get(), this._args[1]._get(), this._args[4]._get(), this._args[3]._get());
    };
  },
  $lt() {
    this.op = '$lt';
    ...
    this.native3 = function () {
      return [
        `(function () {`,
        `var v = ${this._args[0].native3()}`,
        `if (!v) return false;`,
        `for (var i=0; i<v.length; i++ ) {`,
        `if (v[i]<${this._args[1].native3()}) return true;`,
        `}`,
        `return false;})()`
      ].join(`\n`);
    };
  ...
  $in: function () {
    this.op = "$in";
    this._index = function (index) {
      var u = _(this._args.slice(1)).map(function (v) {
        return v._get();
      }).uniq().value();
      return _(u).map(function (v) {
        return index.match(v);
      }).flattenDeep().value();
    },
      this._ex = function (f) {
        if (this.fields()[f] != null)
  ...
  $where: function () {
    ...
    this.native = this.native3 = function () {
      var v = this._args[0]._get();
      if (_.isFunction(v))
        return '(' + v + ').call(obj)';

      if (v instanceof tdb.Code) {
        if (_.isFunction(v.code))
          return '(function () { with (' + JSON.stringify(v.scope) + ') return (' + v.code + ').call(obj); })()';
        return '(function () { with (' + JSON.stringify(v.scope) + ') return (' + v.code + '); }).call(obj)';
      }

      return '(function () { return (' + v + '); }).call(obj)';
    };
  },
  $regex: function () {
    this.op = "$regex";
      ...
      if (op != null) {
        ...
        if (_.isPlainObject(v)) {
          ...
          n._args = [
            new field(k),
            new value(v.source),
            new value((v.global ? 'g' : '') + (v.ignoreCase ? 'i' : '') + (v.multiline ? 'm' : ''))
          ];
          args.push(n);
        } else {
          n = new ops["$eq"]();
